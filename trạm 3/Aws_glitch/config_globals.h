...
inline void Init();
inline void eeprom();
inline void arduinoOTA();
inline void connectAWS();
inline void readData();
inline void display();
inline void tube_impulse();
inline void publishMessage();
inline void tempEvent();
inline void readData();
...
inline void messageReceived(char *topic, byte *payload, unsigned int length) {
...
inline void connectAWS() {
  ...
  int wifi_retry = 0;
  while (WiFi.status() != WL_CONNECTED && wifi_retry < 40) { // Timeout ~20s
    delay(500);
    Serial.print(".");
...
// peripherals
inline void changedname(const char *message, size_t length) {
  DynamicJsonDocument doc(1024);
  deserializeJson(doc, message);
  address = String(doc["namec"]);
  Serial.println("address");
}
inline void storagebysec(const char *message, size_t length) {
  wr = 0;
  Serial.print("storagebysec");
}
inline void storageby1min(const char *message, size_t length) {
  wr = 1;
  Serial.print("storageby1min");
}
inline void storageby5min(const char *message, size_t length) {
  wr = 5;
  Serial.print("storageby5min");
}
inline void connectEvent(const char *payload, size_t length) {
  StationDoc["id"] = "";
...
inline void display() {
  ...
  display_obj.print("T: ");
  display_obj.print(tempC, 1);
  display_obj.print("C");
  display_obj.setCursor(64, 0);
  display_obj.print("H: ");
  display_obj.print(humi, 1);
  display_obj.print("%");

  // Dòng 2: CPS và uSv/h
  display_obj.setCursor(0, 16);
  display_obj.print("CPS: ");
  display_obj.print(cps, 2);
  display_obj.setCursor(0, 28);
  display_obj.print("uSv/h: ");
  display_obj.print(uSv, 3);

  // Dòng 3: Tổng đếm và thời gian
  time_t now_disp =
      time(nullptr); // Tạo biến thời gian cục bộ để tránh xung đột
  struct tm *ptm = localtime(&now_disp);
  char time_str[9];
  strftime(time_str, sizeof(time_str), "%H:%M:%S", ptm);

  display_obj.setCursor(0, 48);
  display_obj.print("Cnt: ");
  display_obj.print(counts); // Hiển thị tổng số đếm
...
// sensors
inline ICACHE_RAM_ATTR void tube_impulse(void) { counts++; }
